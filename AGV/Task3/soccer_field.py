...
class Field:
    ...
    def G(self, x, u):
        """Compute the Jacobian of the dynamics with respect to the state. Mathematical derivation has been attached in the resource file."""
        ...
        return G
    def V(self, x, u):
        """Compute the Jacobian of the dynamics with respect to the control. Mathematical derivation has been attached in the resource file."""
        prev_x, prev_y, prev_theta = x.ravel()
        rot1, trans, rot2 = u.ravel()
    ...
    def H(self, x, marker_id):
        """Compute the Jacobian of the observation with respect to the state. Mathematical derivation has been attached in the resource file."""
        prev_x, prev_y, prev_theta = x.ravel()
        new_x = self.MARKER_X_POS[marker_id]
        new_y = self.MARKER_Y_POS[marker_id]
        euclid_dist = ((new_x-prev_x)**2+(new_y-prev_y)**2)
        H = np.array([(new_y-prev_y)/euclid_dist,(prev_x-new_x)/euclid_dist, -1])
        return H
